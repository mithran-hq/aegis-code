...
Set a stable path to the skill CLI (default `AEGIS_HOME` is `~/.aegis`):

```
export AEGIS_HOME="${AEGIS_HOME:-$HOME/.codex}"
export IMAGE_GEN="$AEGIS_HOME/skills/.system/imagegen/scripts/image_gen.py"
```
...
- Built-in-first transparent image workflow: `SKILL.md` and `$AEGIS_HOME/skills/.system/imagegen/scripts/remove_chroma_key.py`
