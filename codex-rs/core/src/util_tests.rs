...
fn resume_command_prefers_name_over_id() {
    ...
    assert_eq!(command, Some("aegis resume my-thread".to_string()));
}
...
fn resume_command_with_only_id() {
    ...
    assert_eq!(
        command,
        Some("aegis resume 123e4567-e89b-12d3-a456-426614174000".to_string())
    );
}
...
fn resume_command_quotes_thread_name_when_needed() {
    ...
    assert_eq!(
        command,
        Some("aegis resume -- -starts-with-dash".to_string())
    );

    let command = resume_command(Some("two words"), /*thread_id*/ None);
    assert_eq!(command, Some("aegis resume 'two words'".to_string()));

    let command = resume_command(Some("quote'case"), /*thread_id*/ None);
    assert_eq!(command, Some("aegis resume \"quote'case\"".to_string()));
}
