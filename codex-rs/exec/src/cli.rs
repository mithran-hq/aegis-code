...
#[command(
    version,
    override_usage = "aegis exec [OPTIONS] [PROMPT]\n       aegis exec [OPTIONS] <COMMAND> [ARGS]"
)]
pub struct Cli {
    ...
    /// Do not load `$AEGIS_HOME/config.toml`; auth still uses `AEGIS_HOME`.
...
struct ResumeArgsRaw {
    ...
    // so "aegis resume --last <prompt>" treats the positional as a prompt, not a session id.
