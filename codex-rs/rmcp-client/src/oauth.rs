...
//!
//! If the keyring is not available or fails, we fall back to AEGIS_HOME/.credentials.json which is consistent with other coding CLI agents.
...
mod tests {
    ...
    impl TempCodexHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create AEGIS_HOME temp dir");
            unsafe {
                std::env::set_var("AEGIS_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
