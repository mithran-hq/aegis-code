...
Quickstart for running and hitting `aegis app-server`.
...
```bash
# 1) Build debug aegis binary
cargo build -p codex-cli --bin aegis

# 2) Start websocket app-server in background
cargo run -p codex-app-server-test-client -- \
  --aegis-bin ./target/debug/aegis \
...
```bash
cargo run --bin aegis-app-server-test-client -- \
...
```bash
cargo run --bin aegis-app-server-test-client -- thread-resume <THREAD_ID>
```
