...
pub enum Command {
    /// Submit a new cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a cloud task.
    Status(StatusCommand),
    /// List cloud tasks.
    List(ListCommand),
    /// Apply the diff for a cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in cloud.
    ...
    /// Git branch to run in cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
