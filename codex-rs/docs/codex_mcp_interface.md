...
- Server binary: `aegis mcp-server` (or `aegis mcp-server`)
...
```bash
aegis mcp-server | your_mcp_client
```
...
Use the separate `aegis mcp` subcommand to manage configured MCP server launchers in `config.toml`.
