...
//!
//! The history is stored at `~/.aegis/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.aegis`.
...
pub async fn append_entry(
    ...
    // Resolve `~/.aegis/history.jsonl` and ensure the parent directory exists.
